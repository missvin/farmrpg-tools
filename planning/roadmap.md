...
- Item-level goal and source planning
  ...
  - multi-source Buddy item-page parser and reviewed promotion fan-out (`BL-245`, `BL-246`, shipped), current-universe icon refresh (`BL-250`, shipped), and source promotion umbrella (`BL-251`, in progress) split into recipe (`BL-259`), drop-rate (`BL-260`), openable guardrail/promotion (`BL-261`, `BL-262`), source-hint (`BL-263`), alias (`BL-264`), and completion-audit (`BL-265`) child rows
